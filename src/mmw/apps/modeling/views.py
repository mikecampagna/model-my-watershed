...
def start_gwlfe(request, format=None):
    ...
    Takes either the job UUID id of a successful mapshed run as
    'mapshed_job_id', or the serialized GMS, 'model_input'
    ...
    mapshed_job_id = request.POST.get('mapshed_job_uuid', None)

    if mapshed_job_id:
        mapshed_job = get_object_or_404(Job, uuid=mapshed_job_id)
